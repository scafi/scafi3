...
object FieldUtilsLibrary:
  ...
  /**
   * Returns the `data` value associated with the neighbour minimising `key`, excluding the local device. Total: returns
   * `default` for an empty neighbourhood. Ties broken by the smaller device identifier.
   *
   ...
   * @see
   *   [[maxHoodSelector]] for the dual operation
   */
  def minHoodSelector[K: Ordering, V](using
    ...
    (key, data, language.device).mapN { (k, v, id) => (k, id, v) }.withoutSelf.minByOption { (k, id, _) => (k, id) }
      .map(_._3)
      .getOrElse(default)

  /**
   * Returns the `data` value associated with the neighbour maximising `key`, excluding the local device. Total: returns
   * `default` for an empty neighbourhood. Ties broken by the larger device identifier.
   *
   * @param key
   *   the shared field of keys used to rank neighbours
   * @param data
   ...
   * @see
   *   [[minHoodSelector]] for the dual operation
   */
  def maxHoodSelector[K: Ordering, V](using
      language: AggregateFoundation,
  )(using
      Ordering[language.DeviceId],
  )(
    ...
    (key, data, language.device).mapN { (k, v, id) => (k, id, v) }.withoutSelf.maxByOption { (k, id, _) => (k, id) }
      .map(_._3)
      .getOrElse(default)

  /**
   * Key-wise merge of map-valued neighbour contributions. On key collision the `overwrite` policy decides the surviving
   * value: `overwrite(existing, incoming)`.
   *
   * @param maps
   ...
   * @see
   *   [[FoldingLibrary]] for simple `fold`/`foldWithoutSelf` patterns
   */
  def mergeHood[K, V](using
      language: AggregateFoundation,
  )(
