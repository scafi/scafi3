...
object StringSharedDataOps:
  extension(using lang: AggregateFoundation)(data: lang.SharedData[String])
    ...
    def substring(beginIndex: Int, endIndex: Int): lang.SharedData[String] = data.mapValues(_.substring(beginIndex, endIndex))
    ...
    def replaceAll(regex: String, replacement: String): lang.SharedData[String] = data.mapValues(_.replaceAll(regex, replacement))
